...
class Berita_model extends CI_Model
{
    ...
    function getData($limit=10,$mulai=0,$q,$jenis="Halaman Dinamis"){
        if($jenis=="Halaman Dinamis") $this->db->select("*,DATE_FORMAT(post_tglpublish,'%d/%m/%Y') as post_tglpublish,CONCAT(DATE_FORMAT(`post_tanggal`,'%d'),'/',DATE_FORMAT(`post_tanggal`,'%m'),'/',DATE_FORMAT(`post_tanggal`,'%Y'),'/',post_link)  AS post_link");
        else $this->db->select("*,DATE_FORMAT(post_tglpublish,'%d/%m/%Y') as post_tglpublish");
