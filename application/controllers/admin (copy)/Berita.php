...
class Berita extends CI_Controller {
    ...
    function index(){
        ...
        if(!empty($priv)){
            $field=array('kategori_nama','post_judul', 'post_link','post_tglpublish','post_status');
            ...
            $data=array(
                ...
                'content'=> $this->load->view('admin/berita_index',$res,true),
    ...
    function getdata()
    {
        ...
        if(!empty($priv)){
            ...
            $response = array(
                ...
                'message'   => "OK",
                'start'     => $mulai,
                'row_count' => $this->Berita_model->countData($q),
                'limit'     => $limit,
                'data'      => $this->Berita_model->getData($limit, $mulai, $q),
            );
        }else{
    ...
    function simpan(){
        ...
        if(!empty($priv)){
            ...
            $post_link=str_replace(' ','-',strtolower($this->input->post('post_judul')));
            ...
            if(empty($row)){
                ...
                $create_dir = "public/img/blog/original";
                ...
                if($res['status']==true) {
                    ...
                    $error[]=$this->Berita_model->_file_resize($create_dir."/" .$post_image, './public/img/blog/thumb/THUMB_' .$post_image, 300,300);
                    $icon[]=$this->Berita_model->_file_resize($create_dir."/" .$post_image, '../public/img/blog/icon/ICON_' .$post_image, 50,50);
                }else $post_image="";

                $isi=str_replace('../public/img',base_url()."public/img",$this->input->post('post_isi'));
                $data=array(
                    'post_judul'=>$this->input->post('post_judul'),
                    'post_kategori_id'=>$this->input->post('post_kategori_id'),
                    'post_jenis'=>'Halaman Dinamis',
                    'post_isi'=>$isi,
                ...
                $this->form_validation->set_rules('post_kategori_id', 'Kategori', 'required');
                // $this->form_validation->set_rules('post_status', 'Status berita', 'required');
    ...
    function upload(){
         ...
         ***************************************************/
        $accepted_origins = array("http://localhost", "http://192.168.137.1", "https://baznaskabtanahdatar.org");

        /*********************************************
         * Change this line to set the upload folder *
         *********************************************/
        $imageFolder = "public/img/berita/";
